...
if __name__ == "__main__":
    ...
    destination_images_path = (
        current_path + "/" + datasetPath.split("/")[-1] + "_Images_KITTI"
    )
    destination_annotations_path = (
        current_path + "/" + datasetPath.split("/")[-1] + "_Annotations_KITTI"
    )
    ...
    for datasetImgFile in listdir(datasetPath):
        if isfile(join(datasetPath, datasetImgFile)):
            ...
            while 1:
                ...
                cv2.imshow(
                    "image", cv2.addWeighted(img + mask + mask_ref, 0.7, img, 0.3, 0)
                )
                ...
                if k == 27:  # Stop annotating the dataset (Esc key)
                    ...
                    logf.write(
                        "Ending the annotation process for {0}\n".format(filepath)
                    )
                    break
            ...
            if not (len(kitti_data) == 0) and not (cancel_check):
                ...
                for obj in kitti_data:
                    ...
                    annotation_str = (
                        "%s %.2f %.0f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f\n"
                        % (
                            obj["label"],
                            0,
                            0,
